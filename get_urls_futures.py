            ...
            try:
                ...
                WebDriverWait(browser, 5).until(
                    presence_of_element_located(
                        (By.XPATH, '//*[@id="format"]/optgroup[1]/option[8]')
                    )
                )
    ...
    def run_process(self, i, output_filename):
        ...
        if self.connect_to_base(browser, self.url):
            ...
            try:
                ...
                dl_url = browser.find_element_by_xpath(
                    downloadbutton_xpath
                ).get_attribute("href")
    ...
    def scrape_em(self):
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            for i in range(len(self.song_urls_df)):
                futures.append(executor.submit(self.run_process, i, output_filename))
        wait(futures)
        toc = time()
        tictoc = toc - tic
...
if __name__ == "__main__":
    scraper = FutureScraper(
        csv_name="only_tracks.csv", start=0, end=10, headless=False, threads=5
    )
    scraper.scrape_em()
